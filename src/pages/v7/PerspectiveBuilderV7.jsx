...
import ProcessDetailView from '../v4/process/ProcessDetailView';
import StudioL0NavV6 from '../v6/shell/StudioL0NavV6';
import StudioL1Nav from '../v5/shell/StudioL1Nav';
import { getProcessSeed } from '../v4/data/processPageData';
import {
  CONTEXT_BROWSE_TAB_ID,
  NEW_PERSPECTIVE_TAB_ID,
  assetKey,
  tabIconForType,
} from '../v5/shell/studioAssets';
import { InventoryPlacementProvider } from '../v6/inventory/InventoryPlacementContext';
...
function createNewPerspectiveTab() {
  return {
    id: NEW_PERSPECTIVE_TAB_ID,
    label: 'New Perspective',
    type: 'refine',
    icon: 'perspective',
    assetKey: 'perspective:new',
    payload: {
      objects: [],
      events: [],
...
function PerspectiveBuilderV7Shell({
  onVersionChange,
  workspaceTabs,
  setWorkspaceTabs,
  activeTabId,
  ...
  const handleAddToNewPerspective = useCallback(
    ...
    [contextByTab, openRefineTab, exitContextSelection],
  );
  ...
  const handleCreateFromEntireProcess = useCallback(
    (tabId) => {
      ...
      openRefineTab(
        ...
        { sourceProcessId: tab?.payload?.processId },
      );
    ...
    if (tab.type === 'refine' && tab.payload) {
      return (
        <PerspectiveRefineView
          ...
          initialSourceProcessId={tab.payload.sourceProcessId}
        />
      );
    }
  ...
  return (
    <div className={styles.app}>
      ...
      <div className={styles.bodyRow}>
        <StudioL0NavV6 onVersionChange={onVersionChange} />
...
export default function PerspectiveBuilderV7({ onVersionChange }) {
  const [workspaceTabs, setWorkspaceTabs] = useState([
    createBrowseTab(),
    createNewPerspectiveTab(),
  ]);
  const [activeTabId, setActiveTabId] = useState(NEW_PERSPECTIVE_TAB_ID);
  ...
  return (
    <InventoryPlacementProvider>
      <PerspectiveBuilderV7Shell
