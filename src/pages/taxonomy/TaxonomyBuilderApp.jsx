...
function TaxonomyBuilderShell({ onHome, initialObjectTypeId = 'jira-issue' }) {
  ...
  const subtypeEntriesByObjectId = useMemo(
    () => buildSubtypeEntriesByObjectId(getSplitState, getAllSubtypeEntries),
    [generatedTaxonomies, getSplitState, getAllSubtypeEntries],
  );

  const l1Sections = useMemo(
    () => buildL1Sections(generatedTaxonomies, subtypeEntriesByObjectId),
    [generatedTaxonomies, subtypeEntriesByObjectId],
  );
                ...
                >
                  {tab.type === 'object-type' ? (
                    <TaxonomyRefineView
                      ...
                      onTaxonomyGenerated={(taxonomy) => openTaxonomyTab(taxonomy.id)}
