...
import { getFacilitySubtypeSpecializedAttributes } from '../../../data/facilitySubtypeAttributes';
...
export default function SubtypeDetailPanel({
  ...
  const specializedAttributes =
    objectTypeId === 'facility'
      ? getFacilitySubtypeSpecializedAttributes(subtype.matchingValue ?? subtype.label)
      : [];
  ...
  return (
    <div className={panelClass}>
      <div className={styles.detailBody}>
        ...
        {objectTypeId === 'facility' ? (
          <div className={styles.editableBlock}>
            <p className={styles.editableLabel}>Inherited from {objectType.name}</p>
            <ul className={styles.attributeChipList}>
              {objectType.inheritedAttributes.map((name) => (
                <li key={name}>{name}</li>
              ))}
            </ul>
          </div>
        ...
        {specializedAttributes.length ? (
          <div className={styles.editableBlock}>
            <p className={styles.editableLabel}>Specialised on this subtype</p>
            <ul className={styles.attributeChipList}>
              {specializedAttributes.map((attr) => (
                <li key={attr.name}>
                  {attr.name}
                  <span className={styles.attributeTypeHint}>{attr.dataType}</span>
                </li>
              ))}
            </ul>
          </div>
