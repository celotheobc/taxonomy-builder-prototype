...
import { getObjectType, getObjectTypeTableAttributes } from '../../../data/mockObjectTypes';
...
function SubtypeCountBadge({ count, onClick }) {
  if (!count) return null;
  const label = `${count} subtype${count === 1 ? '' : 's'}`;
  if (!onClick) return <span className={styles.badgeSubtypes}>{label}</span>;
  return (
    <button
      type="button"
      className={styles.badgeSubtypesBtn}
      onClick={(event) => {
...
export default function AttributesBindingsSection({
  ...
  const tableAttributes = getObjectTypeTableAttributes(objectType);
  ...
  return (
    ...
    >
      <div className={styles.tabBar} role="tablist">
        ...
        >
          Attributes ({tableAttributes.length})
        </button>
        <button
          type="button"
          role="tab"
