...
export function TreeGuides({ levels, ancestorContinues = [], isLast }) {
          ...
          <span key={col} className={styles.treeGuideCol}>
            ...
            {isConnectorCol ? (
              <>
                ...
                />
                <span className={styles.treeGuideHoriz} />
              </>
            ) : null}
          </span>
        );
...
export function SubtypeRow({
  ...
  const isHeaders = presentation === 'headers';
  const headersIndent = 16 + branchDepth * 20;

  return (
    <div
      className={`${styles.hierarchyRow} ${isHeaders ? styles.hierarchyRowHeaders : ''} ${!readOnly ? styles.hierarchyRowWithActions : ''} ${selectedId === node.id ? styles.hierarchyRowSelected : ''}`}
    >
      {!isHeaders ? (
      ...
      <div
        ...
        style={isHeaders ? { paddingLeft: headersIndent } : undefined}
      >
        {hasChildren ? (
          <button
            type="button"
        ...
        <div className={styles.subtypeMain}>
          {!isHeaders ? <span className={styles.subtypeIcon} aria-hidden /> : null}
        ...
        {!readOnly ? (
          <div className={styles.rowActions}>
            {canSplit ? (
              <button type="button" className={styles.rowIconBtn} aria-label={`Split ${node.label}`} onClick={() => onSplit(node)}>
                <SplitIcon />
              </button>
            ) : null}
